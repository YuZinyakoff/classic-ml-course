...
В этой главе рассматривается обычный метод наименьших квадратов (*ordinary least squares*, OLS). Статистический вывод — стандартные ошибки, доверительные интервалы и проверки гипотез — в основную задачу недели не входит.
...
```math
L_0=\lbrace c\mathbf 1:\;c\in\mathbb R\rbrace .
```
...
Формула (16) описывает аналитическое решение при линейно независимых столбцах. В практических вычислениях матрицу обычно не обращают явно; к этому вернёмся в конце главы.

### 6.2. Точная мультиколлинеарность

Пусть $x_2=2x_1$. Обозначим изменения коэффициентов через $\Delta\beta_1$ и $\Delta\beta_2$. Чтобы прогнозы не изменились, необходимо

```math
\Delta\beta_1x_1+\Delta\beta_2x_2
=
(\Delta\beta_1+2\Delta\beta_2)x_1
=0.
```

Следовательно, $\Delta\beta_1+2\Delta\beta_2=0$. Положим произвольное $\Delta\beta_2=t$; тогда $\Delta\beta_1=-2t$. Поэтому коэффициенты можно изменить так:
...
Число $t$ выбирается произвольно, но соотношение между изменениями коэффициентов продиктовано зависимостью $x_2=2x_1$. Если изменить коэффициенты иначе, так что $\Delta\beta_1+2\Delta\beta_2\ne0$, прогнозы изменятся.
...
Под новым сочетанием признаков здесь понимается не просто новый объект, а комбинация значений, которая не встречалась или была плохо представлена в обучающей выборке. Например, если при $x_1=10$ на train обычно наблюдалось $x_2\approx20$, объект с $x_1=10$ и $x_2=7$ лежит далеко от привычного соотношения. Для двух наборов коэффициентов, отличающихся на $(-2t,t)$, разность прогнозов на таком объекте равна

```math
-2tx_1+tx_2=t(x_2-2x_1).
```

На обучающих объектах эта величина близка к нулю, поскольку $x_2\approx2x_1$. На новом сочетании признаков компенсация может исчезнуть, и прогнозы заметно разойдутся.
...
- **NIST/SEMATECH, Section 4.4** — расширенный набор диагностических графиков и ограничения их интерпретации.
