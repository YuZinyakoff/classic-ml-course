...
Магистерский курс рассчитан на 8 встреч: семь содержательных занятий и
защиту проектов на восьмой встрече. На первой неделе мы подробно разбираем
общую схему эксперимента; затем несколько связанных тем изучаем вместе.

1. **Осмысленный ML-эксперимент**
   Постановка задачи • момент прогноза • train/test • бейзлайн •
   недообучение и переобучение • модель как объект Python.
2. **Линейная модель, функция потерь и обучение**
   МНК • матричная запись • остатки • функции потерь • градиентный спуск.
3. **Вероятностная классификация и решение**
   Логистическая регрессия • вероятности • метрики • порог решения •
   калибровка • короткое расширение к нескольким классам.
4. **Выбор модели и честный эксперимент**
   Регуляризация • validation и cross-validation • обучаемые преобразования •
   `Pipeline` • утечки • kNN как короткий контраст линейной модели.
5. **Деревья и Random Forest**
   Разбиения • сложность дерева • нестабильность • bootstrap • усреднение.
6. **Градиентный бустинг и современные реализации**
   Последовательное исправление прогноза • псевдоостатки •
   CatBoost, XGBoost и LightGBM.
7. **Настройка, диагностика и эксперимент целиком**
   Поиск гиперпараметров • анализ ошибок • интерпретация • ограничения.
8. **Защита проектов**
   Постановка • воспроизводимый эксперимент • результаты • ограничения.

### Как складывается оценка

Максимальная оценка равна 10 баллам:

- **домашняя работа 1 — 2 балла**;
- **домашняя работа 2 — 2 балла**;
- **проект — 4 балла**;
- **активность/посещаемость — 2 балла**.

Итоговый экзамен не планируется.
...
представить результат группе на восьмой встрече. Предлагаемые направления:
...
Подробные требования, формат защиты и критерии оценивания будут объявлены
отдельно.
...
- оценка складывается из двух домашних работ, проекта и активности/посещаемости;
  коллоквиума и отдельного итогового экзамена нет.
...
![Момент t0 отделяет доступную историю клиента от будущих событий и 30-дневного горизонта прогноза](../../../../bachelors/2026/week-01/lecture/assets/prediction-moment.svg)
...
![Десять объектов разделены на восемь обучающих и два тестовых](../../../../bachelors/2026/week-01/lecture/assets/train-test-split.svg)
...
![Сто объектов: 95 объектов класса 0 и 5 объектов класса 1](../../../../bachelors/2026/week-01/lecture/assets/class-imbalance.svg)
...
пороги решений будут центральной темой третьей недели.
...
![Сырые данные двух классов в форме переплетённых полумесяцев без нанесённой модели](../../../../bachelors/2026/week-01/lecture/assets/classification-data.svg)
...
![Одинаковые данные и три области решений показывают недообучение, разумную гибкость и переобучение](../../../../bachelors/2026/week-01/lecture/assets/classification-complexity.svg)
...
![Концептуальные кривые: обучающая ошибка монотонно падает, ошибка на новых данных имеет минимум и затем растёт](../../../../bachelors/2026/week-01/lecture/assets/complexity-errors.svg)
...
![Гладкая зависимость, шумные обучающие наблюдения и три новых наблюдения без нанесённой модели](../../../../bachelors/2026/week-01/lecture/assets/regression-data.svg)
...
![Три полиномиальные модели; оранжевые ромбы обозначают новые значения, пустые квадраты — прогноз каждой модели](../../../../bachelors/2026/week-01/lecture/assets/regression-complexity.svg)
...
![Три модели по-разному продолжают зависимость правее максимального обучающего x](../../../../bachelors/2026/week-01/lecture/assets/regression-extrapolation.svg)
